...
using vehicle_vision_attitude_s                         = vehicle_attitude_s;
 ...
 actuator_controls_s			ORB_actuator_controls_public;
  ...
  sensor_accel_s			ORB_sensor_accel_public[ORB_MULTI_MAX_INSTANCES];
	sensor_accel_1_s    ORB_sensor_accel_1_public[ORB_MULTI_MAX_INSTANCES];
...
void orb_helper_init(void)
{
  ...
  std::memset(&ORB_sensor_accel_1_public, 0, ORB_MULTI_MAX_INSTANCES*sizeof(sensor_accel_1_s));
...
void get_orb_name(const int serial, char * name)
{
  ...
  else if (serial == ORB_sensor_accel_1)
  {
      std::strcpy(name, (ORB_ID(sensor_accel_1))->o_name);
  }
...
int  get_orb_serial(const char * name)
{
  ...
  else if (0 == std::strcmp(name, (ORB_ID(sensor_accel_1))->o_name))
  {
      return ORB_sensor_accel_1;
  }
...
orb_id_t  get_orb_according_to_serial(const int serial)
{
  ...
  else if (0 == std::strcmp(orb_name, (ORB_ID(sensor_accel_1))->o_name))
  {
      return ORB_ID(sensor_accel_1);
  }
...
void  *get_orb_public_according_to_serial_and_instance(const int serial, const int instance)
{
  ...
  else if (is_multi && 0 == std::strcmp(orb_name, (ORB_ID(sensor_accel_1))->o_name))
  {
        return &(ORB_sensor_accel_1_public[instance]);
  }
  ...
  {
      return &(ORB_vehicle_gps_position_public);
  }
