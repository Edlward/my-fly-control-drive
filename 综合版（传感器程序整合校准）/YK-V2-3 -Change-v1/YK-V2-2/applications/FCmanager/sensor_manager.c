...
void sensor_collect(void)
{
	...
	if(sensor_bmi088_ready()){
		if(sensor_get_bmi088_data(&accel,&gyro) == RT_EOK){			
			...
			_sensors.accelerometer_m_s2[0] = (float)accel.x/32768.0f*3*CONSTANTS_ONE_G;
			_sensors.accelerometer_m_s2[1] = (float)accel.y/32768.0f*3*CONSTANTS_ONE_G;
			_sensors.accelerometer_m_s2[2] = (float)accel.z/32768.0f*3*CONSTANTS_ONE_G;
...
//			LOG_RAW("_gyrox: \t  %f \t  _gyroy: \t  %f  \t  _gyroz: \t  %f  \n",_sensors.gyro_rad[0],_sensors.gyro_rad[1],_sensors.gyro_rad[2]);
		}
	}
	...
	if(sensor_mlx90393_ready()){
		if(sensor_get_mlx90393_data(&mlx90393_data) == RT_EOK){	
			...
			//LOG_RAW("_mag.x: \t %f  \t _mag.y:  \t %f  \t _mag.z: \t %f  \n",_mag.x,_mag.y,_mag.z);
