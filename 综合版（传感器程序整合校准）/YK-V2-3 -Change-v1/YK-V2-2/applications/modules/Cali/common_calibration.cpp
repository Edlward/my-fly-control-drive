...
enum detect_orientation_return detect_orientation(int cancel_sub, int accel_sub,bool lenient_still_position)
{
	...
	while (true) 
	{
		...
		if(update)
		{
//			LOG_RAW("sensor_combined  update\n");
...
//      LOG_RAW("accel_disp[0]:%f  accel_disp[1]:%f  accel_disp[2]:%f \n",accel_disp[0],accel_disp[1],accel_disp[2]);
...
//				LOG_RAW("not still\n");
	...
	if (fabsf(accel_ema[0]) < accel_err_thr &&
	    ...
	    fabsf(accel_ema[2]) < accel_err_thr) 
	{
		LOG_RAW("right side down finish \n");
	...
	if (fabsf(accel_ema[0]) < accel_err_thr &&
	    ...
	    fabsf(accel_ema[2] + CONSTANTS_ONE_G) < accel_err_thr) 
	{
		LOG_RAW("level finish \n");
		return DETECT_ORIENTATION_RIGHTSIDE_UP;        // [ 0, 0, -g ]
	}


	LOG_RAW("Can't detect orientation\n");
		...
		bool	lenient_still_position)
{
	...
	while (true) {
		...
		if (side_complete_count == detect_orientation_side_count) {
			...
			LOG_RAW("detect orientation over\n");
			break;
		}
