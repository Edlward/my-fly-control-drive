...
struct rt_ringbuffer* _mav_serial_rb;
...
//					uint32_t now = hrt_absolute_time_ms();
...
{
	uint16_t len;
	len = rt_ringbuffer_data_len(_mav_serial_rb);
	if (!len)
		return 0;
	size = size > len ? len : size;
	return rt_ringbuffer_get(_mav_serial_rb, data, size);
}
...
void mavproxy_entry(void *parameter)
{
	...
	rt_ringbuffer_init(_mav_serial_rb,_mav_serial_buffer, MAV_SERIAL_BUFFER_SIZE);
