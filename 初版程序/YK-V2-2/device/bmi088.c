...
{	
	...
	if(rslt == BMI08X_OK)
	{
		rt_int16_t temp_x = accel_bmi088->x/32768.0f*1000*3;
		rt_int16_t temp_y = accel_bmi088->y/32768.0f*1000*3;
		rt_int16_t temp_z = accel_bmi088->z/32768.0f*1000*3;
		//BMI_TRACE("acc X:%d   acc Y:%d   acc Z:%d !\r\n", temp_x,temp_y,temp_z);
	}
	else
	{
	...
	if(rslt == BMI08X_OK)
	{
		rt_int16_t temp_x = gyro_bmi088->x*30.5f;
		rt_int16_t temp_y = gyro_bmi088->y*30.5f;
		rt_int16_t temp_z = gyro_bmi088->z*30.5f;
		//BMI_TRACE("gro X:%d   gro Y:%d   gro Z:%d !\r\n", temp_x,temp_y,temp_z);
	}
	else
	{
...
static void bmi088_publish(orb_advert_t *_accel_adv,orb_advert_t *_gyro_adv,bmi_data *accel_data,bmi_data *gyro_data)
{
	...
	_gyro.x = (float)gyro_data->x*30.5f/1000.0f;
	_gyro.y = (float)gyro_data->y*30.5f/1000.0f;
	_gyro.z = (float)gyro_data->z*30.5f/1000.0f;
