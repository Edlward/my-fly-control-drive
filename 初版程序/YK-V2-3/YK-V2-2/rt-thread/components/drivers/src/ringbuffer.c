...
rt_bool_t rt_ringbuffer_isfull(struct rt_ringbuffer *rb)
{
	if(RT_RINGBUFFER_FULL == rt_ringbuffer_status(rb))
		return RT_TRUE;
	else
		return RT_FALSE;
}

rt_bool_t rt_ringbuffer_isempty(struct rt_ringbuffer *rb)
{
	if(RT_RINGBUFFER_EMPTY == rt_ringbuffer_status(rb))
		return RT_TRUE;
	else
		return RT_FALSE;
}
