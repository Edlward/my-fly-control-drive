...
#define   spib_cs_set_high()  PDout(10)=1;
#define   spib_cs_set_low()   PDout(10)=0;

#define   WL8                 SPI_DATASIZE_8BIT
//#define   WL16                SPI_DATASIZE_16BIT
...
//  fm25v10_data_put(FM25V10_CMD_RDSR << 8);
	fm25v10_data_put(FM25V10_CMD_RDSR);
    fm25v10_data_put(0x00);
  /* get lower byte only */
//  *status = fm25v10_data_get() & 0xff;
    *status = fm25v10_data_get();
	fm25v10_data_get();
  /* deselect all SPIB slaves */
  spib_cs_set_high();
}
...
//  fm25v10_data_put((FM25V10_CMD_WRSR << 8) | *data);
  fm25v10_data_put(FM25V10_CMD_WRSR);
  fm25v10_data_put(*data);
  //spib_wait_till_external_transaction_complete();
...
//  fm25v10_data_put(message);
   fm25v10_data_put(FM25V10_CMD_WRITE);
   fm25v10_data_put( ( address & 0xFF0000 ) >> 16 );
   fm25v10_data_put( ( address & 0xFF00 ) >> 8 );
   fm25v10_data_put ( address & 0xFF );
  //spib_wait_till_external_transaction_complete();
...
//  fm25v10_data_put(message);
    fm25v10_data_put(FM25V10_CMD_READ);
	fm25v10_data_put( ( address & 0xFF0000 ) >> 16 );
	fm25v10_data_put( ( address & 0xFF00 ) >> 8 );
	fm25v10_data_put( address & 0xFF );
  //spib_wait_till_external_transaction_complete();
...
u8 fm25v10_data_get(void) {
  /* Wait for the SPI External Transaction Complete*/
//  //spib_wait_till_external_transaction_complete();
//  return *pRXSPIB;
	u8 rec;
	rec=SPI2_ReadWriteByte(0x00);
	return rec;
}
...
void fm25v10_get_id(byte * ID) {
  ...
  for (index = 0; index < 9; index++) 
  {
    ...
    /* clock SPI with no data sent while get data from FM25V10*/
//    fm25v10_data_put(0);
   ID[index]=SPI2_ReadWriteByte(0x00);
//    /* get data from SPI receive buffer */
...
//	  SN[index] = SPI2_ReadWriteByte(0x00);
  }
...
word fm25v10_test(void) {
  ...
  byte ID[9];
  byte SN[8];
  byte temp_array[10]; // for temporary store data from FRAM
  byte status;
  byte FM25V10_ID[9] = {0x7F, 0x7F, 0x7F, 0x7F, 0x7F, 0x7F, 0xC2, 0x22, 0x08};

  uint reference_array[10]; // array with test data
  uint test_array[10]; // temp array for read
...
//  printf("status_disable:%d\r\n",status);
  ...
  fm25v10_data_write(index, &reference_array[0], WL8, 10);
  ...
  for (index = 0; index < 128; index++) {
    ...
    fm25v10_data_read(index * 10, (uint*)&temp_array[0], WL8, 10);

    //asm(" bit tgl flags FLG3;"); //watchdog
    //asm(" bit tgl flags FLG1;"); //LED

    /* write data from reference array */
    fm25v10_data_write(index * 10, &reference_array[0], WL8, 10);

    //asm(" bit tgl flags FLG3;"); //watchdog
    //asm(" bit tgl flags FLG1;"); //LED

    /* read data from FM25V10 to test array */
    fm25v10_data_read(index * 10, &test_array[0], WL8, 10);

    //asm(" bit tgl flags FLG3;"); //watchdog
    //asm(" bit tgl flags FLG1;"); //LED

    /* write stored data from temp array */
    fm25v10_data_write(index * 10, (uint*)&temp_array[0], WL8, 10);

    //asm(" bit tgl flags FLG3;"); //watchdog
    //asm(" bit tgl flags FLG1;"); //LED

    /* compare data in reference and in test array */
    if (memcmp(&reference_array[0], &test_array[0], 10) != 0)
      result |= 8;
  }
